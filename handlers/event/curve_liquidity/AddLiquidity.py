...
class handler():
    ...
    def compose(self):
        ...
        if (self.tx["args"]["token_amounts"][0] + self.tx["args"]["token_amounts"][1]) / 1e18 > float(os.getenv("SENDING_THRESHOLD_ETH")):
            self.content = '<@&945071604642222110>'
        if (self.tx["args"]["token_amounts"][0] + self.tx["args"]["token_amounts"][1]) / 1e18 > float(os.getenv("RISK_THRESHOLD_ETH")):
            self.send = True
