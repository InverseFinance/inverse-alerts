...
class handler():
    ...
    def compose(self):
        ...
        self.title = "Lending Market : New Repayment event detected for " + str(market_symbol)
        self.fields = [{"name": 'Block Number :', "value": str(f'[{blockNumber}](https://etherscan.io/block/{blockNumber})'),
                   "inline": False},
                  {"name": 'Borrower :', "value": str(
                      f'[{self.tx["args"]["borrower"]}](https://etherscan.io/address/{self.tx["args"]["borrower"]})'),
                   "inline": True},
                  ...
                  {"name": 'Market Address :', "value": str(f'[{address}](https://etherscan.io/address/{address})'),
                   "inline": True},
                  {"name": 'ETH Value :', "value": str(
                      formatCurrency(self.tx["args"]["repayAmount"] / underlying_decimals * underlying_price)),
                   "inline": True},
                  {"name": 'Borrow Amount :',
                   "value": str(formatCurrency(self.tx["args"]["repayAmount"] / underlying_decimals)), "inline": True},
                  {"name": 'Account Borrows :',
                   "value": str(formatCurrency(self.tx["args"]["accountBorrows"] / underlying_decimals)),
                   "inline": True},
                  {"name": 'Total Borrows :',
                   "value": str(formatCurrency(self.tx["args"]["totalBorrows"] / underlying_decimals)), "inline": True},
