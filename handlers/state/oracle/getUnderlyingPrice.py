...
class handler():
    ...
    def compose(self):
        ...
        if abs(self.change) > float(os.getenv('HIGH_RISK_CHANGE')):
            ...
            self.send = True
        elif abs(self.change) > float(os.getenv('LOW_RISK_CHANGE')):
            self.level = 1
            self.color = colors.orange
            self.send = True
        if self.send:
            logging.info(str(self.change) + '% change detected on ' + str(getSymbol(self.web3, getUnderlying(self.web3, self.state_argument))))

            self.webhook = os.getenv('WEBHOOK_MARKETS')
            self.title = str(formatPercent(self.change)) + ' change detected on ' + str(getSymbol(self.web3, getUnderlying(self.web3, self.state_argument))) + ' Oracle'
