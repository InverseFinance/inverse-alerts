...
class Listener(Thread):
    def __init__(self, web3, alert, contract, function, webhook, **kwargs):
...
def handle_message(alert, tx, function, state_results, webhook):
    try:
        ...
        error_webhook = 'https://discord.com/api/webhooks/978701911496728606/l0eSxx1x5_Q_SnCG1Wm1IxbE33Zitx-ZQ4LxOULA-V8zgi2IKXQnQcvNaJZpWqUZ-6gh'
...
def sendWebhook(webhook, title, fields,content, imageurl, color):
    try:
        ...
        embed = f'[{{"fields":{fields},"title": "{title}","content": "{content}","color": "{color}","image":{{"url": "{imageurl}"}}}}]'
